    ...
    return (
        <li className={classNames({ "layout-root-menuitem": props.root, "active-menuitem": active })}>
            {props.root && item.visible !== false && (
                <div className="layout-menuitem-root-text">{translate(item.label)}</div>
            )}
            {(!item.to || item.items) && item.visible !== false ? (
                <>
                    <a
                        ...
                        target={item.target}
                        data-pr-tooltip={translate(item.label)}
                        data-pr-disabled={!(isSlim() && props.root && !layoutState.menuHoverActive)}
                        tabIndex={0}
                        onMouseEnter={onMouseEnter}
                    >
                        <i className={classNames("layout-menuitem-icon", item.icon)}></i>
                        <span className="layout-menuitem-text">{translate(item.label)}</span>
